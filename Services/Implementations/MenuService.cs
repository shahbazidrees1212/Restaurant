...
public class MenuService(AppDbContext db) : IMenuService
{
    ...
    {
        var q = db.MenuItems.Where(x => x.IsAvailable).AsQueryable();
        ...
        return await q.OrderBy(x => x.Category).ThenByDescending(x => x.IsPopular).ToListAsync();
    }
    public async Task<List<MenuItem>> GetPopularAsync(int count=6) => await db.MenuItems.Where(x=>x.IsPopular || x.IsChefSpecial).Take(count).ToListAsync();
    public async Task<List<string>> GetCategoriesAsync() => await db.MenuItems.Select(x=>x.Category).Distinct().OrderBy(x=>x).ToListAsync();

    public async Task<MenuItem?> GetByIdAsync(int id)
    {
        return await db.MenuItems.FirstOrDefaultAsync(x => x.Id == id);
    }
}
